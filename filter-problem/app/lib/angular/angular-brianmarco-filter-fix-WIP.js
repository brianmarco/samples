  ...
  return {
    ...
    compile: function(element, attr, linker) {
      return function($scope, $element, $attr){
        ...
        // START - brianmarco WIP fix for pipe after "track by"
        //
        // Looks like there might be an issue with how the expression is regex'd.  From what I can tell when using an
        // expression of the nature:
        // 
        // _item_ in _collection_ track by _track_by_function_ | filter:search
        //
        // The following variables should end up with the following for things to work:
        //
        // lhs = '_item_'
        // rhs = '_collection_ | filter:search' (essentially ripping out the "track by $index" part)
        // trackByExp = '_track_by_function_'
        //
        // The original regex has problems getting the variables assigned as stated above.  Added logic to break down
        // the expression processing to get the variables their needed values.  I'm pretty sure there's a better way to
        // arrange the patterns.
        //

        var trackByExp, trackByExpGetter, trackByIdFn, lhs, rhs, valueIdentifier, keyIdentifier,
        ...
        if (expression.match(/track\s+by/)) {
            if (expression.match(/.*\|.*/)) {
                match = expression.match(/^\s*(.+)\s+in\s+(.*)\s*track\s+by\s+(.+)\s*(\|.*)+$/);
                rhs = match[2] + ' ' + match[4];
            }
            else {
                match = expression.match(/^\s*(.+)\s+in\s+(.*)\s*track\s+by\s+(.+)$/);
                rhs = match[2];
            }
        ...
        // END - brianmarco DRAFT fix for pipe after "track by"
